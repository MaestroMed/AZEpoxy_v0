...
// Horizontal shift is now handled at the engine level via
// store.anchorOffsetX — see HomepageSwarmTimeline for the desktop
// right-bias used on the home page. Keeping this phase focused on
// pure text shape.
...
export const AZ_PHASE: Phase = {
  ...
  computeTarget(count, _time, mouse, _scroll, out) {
    ...
    // Copy cached targets — then offset slightly based on mouse for a
    // parallax "breath". Horizontal shift (per-page right-bias) lives in
    // the engine's anchorOffsetX pass.
